...
class AccountReporter:
    ...
    def _max_drawdown(
        ...
        *,
        ...
        """
        Maximum historical drawdown from the first account snapshot
        up to the requested snapshot.

        Drawdown is stored in the database as zero or a negative fraction.
        Example:
            0
            -0.012
            -0.035

        Therefore the minimum value is the maximum historical drawdown.
        The returned value is positive:
            -0.035 -> 0.035
        """

        value = session.scalar(
            select(func.min(AccountSnapshot.drawdown)).where(
                AccountSnapshot.strategy_id == strategy_id,
                AccountSnapshot.captured_at <= through,
            )
        )
    ...
    def snapshot_report(
        ...
        *,
        ...
        with Session(self.engine) as session:
            ...
            portfolio = Decimal(
                snap.portfolio_value or 0
            )

            gold = Decimal(
                snap.gold_exposure or 0
            )
            ...
            report: dict[str, Any] = {
                ...
                "captured_at": (
                    ...
                    else snap.captured_at.strftime(
                        "%Y-%m-%d %H:%M"
                    )
                ),
                ...
                # Drawdown of the current snapshot
                "drawdown":
                    snap.drawdown,

                # Maximum historical drawdown
                # from account inception until this snapshot
                "max_drawdown":
                    max_drawdown,

                "active_positions_count":
                    snap.active_positions_count,

                "active_funds": [
                    symbols.get(
                        int(p.current_fund_id),
                        str(p.current_fund_id),
                    )
                    for p in positions
                ],

                "rotations_count": sum(
                    int(p.rotations_count or 0)
                    for p in positions
                ),
            }
